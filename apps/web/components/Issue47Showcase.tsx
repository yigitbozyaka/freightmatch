  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-2">
        <button type="button" onClick={() => pushToast('Marketplace refreshed with 24 new loads.', 'info')} className="rounded border border-amber-500/50 bg-amber-500/10 px-3 py-2 font-mono text-xs uppercase tracking-widest text-amber-300">
          Trigger Info Toast
        </button>
        <button type="button" onClick={() => pushToast('Load sync failed — retrying in 10 seconds.', 'error')} className="rounded border border-[--color-danger]/50 bg-[--color-danger]/10 px-3 py-2 font-mono text-xs uppercase tracking-widest text-[--color-danger]">
          Trigger Error Toast
        </button>
      </div>
