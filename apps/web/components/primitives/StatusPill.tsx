...
type Props = { status: FreightStatus };
...
export function StatusPill({ status }: Props) {
  return (
    <span
      className={`inline-flex items-center rounded-full border px-2.5 py-1 text-[11px] font-mono uppercase tracking-wide ${statusClassMap[status]}`}
    >
      {status}
    </span>
  );
